...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'POST') {
    ...
    const { roundNumber, title, description, imageUrl, nftLink, submittedBy } = req.body;
    if (!title || !description || !imageUrl || !submittedBy) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    try {
      const dbRes = await query(
        `INSERT INTO rounds (round_number, title, description, image_url, nft_link, submitted_by, approved, timestamp)
         VALUES ($1, $2, $3, $4, $5, $6, false, NOW())
         RETURNING *`,
        [roundNumber, title, description, imageUrl, nftLink, submittedBy]
      );
